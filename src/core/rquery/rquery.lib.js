...
/**
 * Representative the RQuery class for working with DOM elements.
 * */
class RQuery {
	/**
	 ...
	 * @returns {RQuery[]} An array of new RQuery instances for the found
	 * elements.
	 * */
	...
	/**
	 * Get or ser the inner HTML of the selected element.
	 ...
	 * setting HTML content, or the current inner HTML when getting.*/
	html(htmlContent) {
	 ...
	 * not provided, the current inner text will be returned.
	 * @returns {RQuery|string} The current RQuery instance for chaining when
	 * setting text content, or the current text when getting.*/
	text(textContent) {
	...
	/**
	 * Add an event listener tho the selected element for the specified
	 * event type.
	 ...
	 * to execute when the event is triggered. The function will receive
	 ...
	 * @return {string|RQuery} - If newValue is provided, returns the RQuery
	 * instance. Otherwise, returns the current value of the input element.
	 * */
	value(newValue) {
	...
	/**
	 * Set attributes and event listeners for number input element.
